...
Try {
	...
	[string]$appName = 'Remote Tools for Visual Studio'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		$exitCode = Execute-Process -Path "$dirFiles\VS_RemoteTools.exe" -Parameters "/install /quiet" -WindowStyle "Hidden" -PassThru
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$exitCode = Execute-Process -Path "$dirFiles\VS_RemoteTools.exe" -Parameters "/uninstall /quiet" -WindowStyle "Hidden" -PassThru
        If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
